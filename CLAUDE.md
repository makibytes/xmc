...
- `BrowseBackend` interface (`broker/backends/queue.go`) - optional, true non-destructive peek cursor (Artemis/RabbitMQ: AMQP copy-mode receiver; NATS: GetMsg by stream sequence; Redis: XRANGE cursor; Azure: native PeekMessages cursor; IBM MQ: MQOO_BROWSE + BROWSE_FIRST/NEXT). Brokers without it fall back to stateless Receive(ack=false), where `peek -n 0` repeats the head message; the native-or-fallback dispatch is centralized in `backends.Browse(ctx, qb, opts)` (mirrors `backends.Request` above), used by both `cmd/receive.go` and `mcp/tools.go`.
...
- `receive.go` - Message receive with timeout, acknowledge/release, selectors, and durable subscriptions; destructive reads (`Acknowledge: true`) go through `link_cache.go`'s `ReceiverCache`
...
- `link_cache.go` - `SenderCache`/`ReceiverCache`: AMQP links are attached once per adapter and reused across calls instead of attach/detach per message (a real cost on every message-loop command: `send -l`, `receive -n 0`, `subscribe`, `move`, `reply`, `forward`, `bridge`). Deliberate carve-out: non-destructive reads (`Acknowledge: false`, i.e. peek) always bypass the cache via `receiveOnce`, replicating the pre-caching attach→receive→release→detach cycle — caching a *released* link keeps it attached with outstanding credit, so the broker prefers redelivering to that same link instead of making the message visible to an independent second peeker. Do not "optimise" peek onto the cache; it was tried and breaks that guarantee (caught by `TestArtemis_QueuePeek`/`TestArtemis_QueueRepeatedPeek`).
...
- `broker/backends/browse.go` - `Browse(ctx, qb, opts) (Browser, error)` (native `BrowseBackend` cursor when the adapter has one, else a stateless-Receive fallback; see the `BrowseBackend` bullet above)
- `broker/backends/common.go` - `RegisterCommonFlags` (server/user/password persistent flags with env-var binding; used by every non-cloud broker entry file)
...
- `cmd/manage.go` - `ManageSpec` + `ObjectType` + `ManageAction` + `BindAction` + `NewManageCommand()` (shared manage subcommand builder with standardised output via `c.OutOrStdout()`; `ObjectType` declares browsable object types with generic `ObjectNode` list; supports list/purge/stats, create/delete/update for queues, topics, exchanges, and bindings, plus create-less asymmetric actions like `UpdateQueue`/`EnableQueue`/`DisableQueue` (Artemis) and `DeleteConsumerGroup` (Kafka — no create counterpart, since Kafka creates groups implicitly); fresh command built per shell invocation for clean IO routing. `ObjectType` also declares the AI-shell sidebar's per-broker hotkey eligibility — `Singular`, `Drain`, `SendViaTopic(nodeKind string) bool`, `Publish`, `ChildKind` — set once by each broker entry file; this is the single source of truth the shared TUI code (`cmd/aikeys.go`, `cmd/aisidebaractions.go`) reads instead of matching on `Label` strings, so a new broker can't silently render a sidebar window with no working hotkeys)
- `cmd/produce.go` - `registerProduceFlags`, `parseProduceFlags`, `runProduce` (shared producer logic for send/publish)
- `cmd/command.go` - `WrapQueueCommand`/`WrapTopicCommand` adapter factories with lazy connection, both thin wrappers over the generic `wrapCommand[T Closeable]`
...
- `cmd/aiclient.go` - `aiClient` interface + provider implementations (Anthropic, OpenAI-compatible, Gemini) via stdlib `net/http`; provider-aware `low`/`medium`/`high` effort maps to each current API's native reasoning controls and falls back to temperature for legacy/custom models; `isReasoningModel` detects OpenAI's o1/o3/o4/gpt-5/codex family to swap `max_tokens`→`max_completion_tokens` and omit `temperature`
...
- `cmd/` - Generic command implementations using backend interfaces
  ...
  - `command.go` - `WrapQueueCommand`/`WrapTopicCommand` adapter factories, thin wrappers over the generic `wrapCommand[T Closeable]`
