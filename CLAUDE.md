...
- Management commands: `manage list`, `manage purge`, `manage stats`, `manage create-queue`, `manage delete-queue`, `manage update-queue`, `manage enable-queue`/`disable-queue` (Artemis), `manage create-topic`, `manage delete-topic`, `manage create-exchange`, `manage delete-exchange`, `manage bind-queue`, `manage unbind-queue` (bind target noun per broker via `BindAction.TargetNoun`: RabbitMQ exchange, Artemis address)
