...
func ReceiveMessage(qMgr ibmmq.MQQueueManager, args ReceiveArguments) (*ibmmq.MQMD, []byte, ibmmq.MQMessageHandle, error) {
	...
	if err != nil {
		return nil, nil, fmt.Errorf("failed to open queue: %w", err)
	}
...
func convertMQTimestamp(date string, time string) time.Time {
	// IBM MQ uses YYYYMMDD and HHMMSSTH format
